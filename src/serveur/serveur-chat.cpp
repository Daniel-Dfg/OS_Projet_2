...
void signalHandler(int signum) {
  std::cerr << "Signal reçu : " << signum
            << " (SIGINT). Fermeture du serveur...\n";
  Server::stop();
}
...
void Server::init(int port) {
  ...
  int opt = 1;
  check_return_value(setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT, &opt, sizeof(opt)), "setsockopt");
  ...
  check_return_value(listen(server_fd, MAX_QUEUE), "listen");

  pollfd server_pollfd = {server_pollfd.fd = server_fd,
                          server_pollfd.events = POLLIN,
                          server_pollfd.revents = 0};
  poll_fds.push_back(server_pollfd); // Ajoute le socket principal à la liste
                                     // des sockets surveillés
}

// Gère l'arrivée d'une nouvelle connexion client
void Server::handleNewConnection() {
  // limite des connexion 
  if (poll_fds.size() - 1 >= MAX_CLIENTS) { // -1 car le socket principal est inclus dans poll
    std::cerr << "Limite de 1000 clients atteinte : connexion refusée.\n";
    return;
  }
  ...
  int client_fd = check_return_value(
      accept(server_fd, (struct sockaddr *)&client_addr, &addr_len), "accept");
  ...
  // Vérification si le pseudo est déjà utilisé, si oui,  on déconnecte le
  // client
  if (name_to_fd.find(client_name) != name_to_fd.end()) {
    std::cerr << "Pseudo déjà utilisé : " << client_name
              << ". Déconnexion du nouveau client.\n";
    string error_message = "[SYSTEM] Pseudo déjà utilisé. Connectez-vous avec un autre pseudo.\n";
    check_return_value(write(client_fd, error_message.c_str(), error_message.size()));
    close(client_fd); // Ferme la connexion pour le client ayant le pseudo en
                      // double
    return;
  }

  // Permet de surveiller si il y a un événment disponible sur la socket du
  // client, push dans le pool des truc à surveillé
  pollfd client_pollfd = {client_pollfd.fd = client_fd,
...
// Gère les messages reçus d'un client, //TODO séparé cette fonction en
// plusieurs partie
void Server::handleClientMessage(int client_fd) {
  ...
  int bytes = check_return_value(read(client_fd, buffer, sizeof(buffer) - 1),
  ...
  if (pos == string::npos) {
    ...
    string error_message = "Erreur : format de message incorrect. Utilisez "
  ...
  if (message_text.size() > MAX_MESSAGE_SIZE) {
    string error = "Erreur : le texte du message dépasse " +
                   std::to_string(MAX_MESSAGE_SIZE) + " octets.\n";
...
void Server::handleDisconnection(int client_fd) {
  ...
  check_return_value(close(client_fd),
                     "fermeture socket client"); // on exit si on arrive pas à
                                                 // fermé la socket client??
...
void Server::sendMessage(const string receiver, const Message &msg) {
  ...
  if (it != name_to_fd.end()) {
    ...
    if (bytesSent == -1) {
      ...
      std::cerr << "Erreur  lors de l'envoi à " << receiver
                << ". Déconnexion...\n";
      handleDisconnection(it->second);
    } else {
      std::cout << "Message envoyé à " << receiver << " : " << msg.getText()
                << std::endl;
    }
  } else {
    // On envoie un message à l'expediteur que l'utilisateur à qui il a envoyé
    // le message n'est pas connecté
    std::cerr << "Cette personne (" << receiver
...
void Server::run() {
  ...
  while (running) {
    ...
    if (ret == -1) {
      if (errno ==
          EINTR) { // Vérifier si le poll a été interrompu par un CTRL + C
        running = false;
        break;
    ...
    for (size_t i = 0; i < poll_fds.size(); i++) {
      ...
      if (poll_fds[i].fd == server_fd) {
        ...
        handleClientMessage(
            poll_fds[i].fd); // Si l'evenement est sur le socket client
      }
    }
  }
...
void Server::stop() {
  if (!running)
    return; // Empêche plusieurs appels
  running = false;
...
int main() {
  ...
  // récuperaiton du port
  int port = 1234; // Port par défaut
  const char* port_env = std::getenv("PORT_SERVEUR");

  if (port_env) {
    try {
      int env_port = std::stoi(port_env);
      if (env_port >= 1 && env_port <= 65535) {
        port = env_port;
      } else {
        std::cerr << "Port invalide dans variable environnement PORT_SERVEUR, utilisation du port 1234 par défaut..";
      }
    } catch (const std::exception&) {
      std::cerr << "Erreur, utilisation du port 1234 par défaut..";
    }
  }
  try {
    Server::start(port); // Démarre le serveur
